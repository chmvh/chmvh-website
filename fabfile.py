...
def configure_nginx():
    ...
    sudo('ln -fs /etc/nginx/sites-available/chmvh-website.conf '
...
def _set_up_ssl():
    ...
    _upload_template(
        'templates/ssl-domain.conf',
        '/etc/nginx/snippets/ssl-{0}.conf'.format(env.host),
        context)
    put('templates/ssl-params.conf', '/etc/nginx/snippets')
